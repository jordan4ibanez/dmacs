...
function jsMain() {
	...
	miniBuffer.interactiveMode(
		(e) => {
			...
			const input = e.key;
			if (input === "Enter") {
				print("you typed: " + minibuffer.mutable);
			...
			if (input === "Backspace") {
				minibuffer.mutable = minibuffer.mutable.slice(0, -1);
			} else {
				minibuffer.mutable = minibuffer.mutable + input;
			}
