...
const elementOrSelector = (el: any) => {
	if (isString(el)) {
		const ele = document!!.querySelector(el as string);
		if (!ele) {
...
const defaultElementStyleFn = (dim: any, size: any, gutSize: any) => {
	const style: any = {};
...
export const Split = (idsOption: any, options = {}): Instance => {
	if (ssr) return {} as Instance;
...
//? https://github.com/nathancahill/split/blob/master/packages/splitjs/index.d.ts
